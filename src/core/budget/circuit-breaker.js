...
class CircuitBreaker {
  ...
  async _acquireStateMutex() {
    this.logger.debug(`[CircuitBreaker] State mutex acquisition requested, current state: ${this.state}`);
    ...
    await currentMutex;
    this.logger.debug(`[CircuitBreaker] State mutex acquired`);
    return unlock;
  }
  ...
  canExecute() {
    ...
    this.logger.debug(`[CircuitBreaker] canExecute check:`, {
      state: this.state,
  ...
  _determineCanExecute(now) {
    ...
    if (this.state === 'OPEN') {
      ...
      if (now >= this.nextAttemptTime) {
        this.logger.info(`[CircuitBreaker] Reset timeout elapsed, allowing test request`, {
   ...
   */
  async recordSuccess() {
    this.logger.info(`[CircuitBreaker] recordSuccess called, current state:`, {
      state: this.state,
      failureCount: this.failureCount,
      successCount: this.successCount
    });
    ...
    try {
      ...
      this.failureCount = 0;
      this.logger.debug(`[CircuitBreaker] Failure count reset to 0`);
      ...
      if (this.state === 'OPEN') {
        ...
        if (now >= this.nextAttemptTime) {
          ...
          this.successCount = 1;
          this.logger.info(`[CircuitBreaker] State transition: OPEN -> HALF_OPEN`, {
            reason: 'success after timeout',
            successCount: this.successCount,
            successThreshold: this.successThreshold
          });
        } else {
          this.logger.warn(`[CircuitBreaker] Success recorded in OPEN state but timeout not elapsed`, {
            now,
            nextAttemptTime: this.nextAttemptTime,
        ...
        this.successCount++;
        this.logger.info(`[CircuitBreaker] Success counter incremented in HALF_OPEN:`, {
          successCount: this.successCount,
          successThreshold: this.successThreshold,
        ...
        if (this.successCount >= this.successThreshold) {
          this.state = 'CLOSED';
          this.successCount = 0;
          this.logger.info(`[CircuitBreaker] State transition: HALF_OPEN -> CLOSED`, {
        ...
        this.state = 'CLOSED';
        this.logger.debug(`[CircuitBreaker] Success recorded in CLOSED state (normal operation)`);
      }

      if (previousState !== this.state) {
        this.logger.info(`[CircuitBreaker] State changed: ${previousState} -> ${this.state}`);
      }
    } finally {
      this.logger.debug(`[CircuitBreaker] State mutex released after recordSuccess`);
      unlock();
    }
  }
   ...
   */
  async recordFailure() {
    this.logger.info(`[CircuitBreaker] recordFailure called, current state:`, {
      state: this.state,
      failureCount: this.failureCount,
    ...
    try {
      ...
      this.logger.info(`[CircuitBreaker] Failure counter updated:`, {
      ...
      if (this.state === 'HALF_OPEN') {
        ...
        this.successCount = 0;
        this.logger.warn(`[CircuitBreaker] State transition: HALF_OPEN -> OPEN`, {
          reason: 'test failed',
      ...
      if (this.failureCount >= this.failureThreshold) {
        this.state = 'OPEN';
        this.nextAttemptTime = this.lastFailureTime + this.resetTimeout;
        this.logger.error(`[CircuitBreaker] State transition: ${previousState} -> OPEN`, {
        ...
        this.logger.debug(`[CircuitBreaker] Failure recorded but threshold not reached:`, {
          state: this.state,
          failureCount: this.failureCount,
          threshold: this.failureThreshold
        });
      ...
      this.logger.debug(`[CircuitBreaker] State mutex released after recordFailure`);
      unlock();
    }
  }
   ...
   */
  reset() {
    this.logger.info(`[CircuitBreaker] Manual reset requested`, {
    ...
    this.logger.info(`[CircuitBreaker] Reset complete, state: CLOSED`);
  }
