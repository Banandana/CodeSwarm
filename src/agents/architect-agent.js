/**
 * Architect Agent
 * Specializes in system architecture and design
 */

const BaseAgent = require('./base-agent');
const { generateArchitectPrompt } = require('./prompts/architect-agent');
const { AgentError } = require('../utils/errors');

class ArchitectAgent extends BaseAgent {
  constructor(agentId, communicationHub, options = {}) {
    super(agentId, 'architect', communicationHub, options);
  ...
  async executeTask(task) {
    ...
    const { systemPrompt, userPrompt, temperature, maxTokens } =
      generateArchitectPrompt(task, context);
 ...
 // Execute file operations
  ...
  /**
   * Validate architect-specific task requirements
   * @param {Object} task
   * @returns {Object}
   */
  validateTask(task) {
    ...
    if (task.agentType && task.agentType !== 'architect') {
      return {
        valid: false,
